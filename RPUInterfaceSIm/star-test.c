...
int chooseDeviceAndChannelID(const char * const descriptionStr,
    STAR_CHANNEL_ID * const pChannelId,
    const STAR_CHANNEL_DIRECTION direction, STAR_DEVICE_ID *deviceId)
{
   // STAR_DEVICE_ID deviceId;
    ...
    *pChannelId = STAR_openChannelToLocalDevice(*deviceId, direction,
    ...
    if ((*pChannelId) == 0U)
    {   
         printf("\nERROR: Unable to open channel. Check link connections or try re-inserting the USB cable in PC.\n");
        fprintf(logFile,"%s", "\nERROR: Unable to open channel\n");
        return 0;
    }
    
   fprintf(logFile,"%s %d %s %d %s %d %s","deviceId--",*deviceId, ", channelNumber--", channelNumber,", pChannelId--", *pChannelId,".\n");
...
//change    pAddressPath = getTransmitPathAddress();
...
//change    pAddressPath[0U] = getTransmitPathAddress();
...
//change    pAddressPath[1U] = getTransmitPathAddress();
