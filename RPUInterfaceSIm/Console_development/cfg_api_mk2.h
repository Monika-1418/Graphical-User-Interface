 ...
 * \ingroup hardwareMk2
 *
 * Reads information about the hardware version of a device and assigns the
 * passed in MK2_HARDWARE_VERSION structure pointer to point to the received
 * data
 *
 *
 * @param       deviceID    Identifier for Mk2 compatible device to get hardware info from
 * @param[out]  pInfo       STAR_CFG_MK2_HARDWARE_INFO that will be updated to contain
 ...
 *
 *
 * @param   info            Hardware information obtained from a a call to CFG_MK2_getHardwareInfo()
 * @param   version         Zero terminated string of length STAR_CFG_MK2_VERSION_STR_MAX_LEN that
 *                          will be updated to contain a string representation of the hardware version.
 * @param   buildDate       Zero terminated string of length STAR_CFG_MK2_BUILD_DATE_STR_MAX_LEN that
 *                          will be updated to contain a string representation of the hardware build date.
 */
void STAR_API_CC CFG_MK2_hardwareInfoToString(
                                STAR_CFG_MK2_HARDWARE_INFO info,
                                _Out_z_cap_c_(STAR_CFG_MK2_VERSION_STR_MAX_LEN) char* version,
                                _Out_z_cap_c_(STAR_CFG_MK2_BUILD_DATE_STR_MAX_LEN) char* buildDate);
 ...
 * @param[out]  period  A pointer to a variable that will be updated to contain
 ...
 * Where \f$Link Clock Rate\f$ is set by the appropriate function (e.g.
 ...
 *       transmit rate permitted by SpaceWire is 2Mbps (i.e. 200Mbps / 100).
 *
 ...
 * \ingroup linksMk2
 *
 * Sets the base transmit clock rate on a given link. The transmit rate of a link is
 * given by:
 *
 *  \f[
 ...
 * hardware version v1.11 and SPLT devices before hardware version v2.0. \n
 ...
 * 2 Mbits/s and 400 Mbits/s.
 *
 * Note that for data rates below 10 Mbits/s, an exact rate may not be achievable
 ...
 *
 * \note This function is currently only compatible with \ref PCIMk2, SpaceWire
 *       Physical Layer Tester and \ref PCIe devices. No check is made by this
 *       function to ensure it is being used with a compatible device.  For
 *       \ref BrickMk2, \ref RouterMk2S and \ref BrickMk3 devices, please use
 ...
 *                      device's clock rate.
 * @param action        Action to be performed.  This can be 'SPW_TRANSMIT_PACKET'
 ...
 *
 * \note This function is currently only compatible with \ref PCIe,
 *       and SpaceWire Physical Layer Tester devices. No check is made by this
 *       function to ensure it is being used with a compatible device.
 *
 ...
 *
 * \note This function is currently only compatible with \ref PCIe,
 *       and SpaceWire Physical Layer TesterT devices. No check is made by this
 *       function to ensure it is being used with a compatible device.
 *
 * \note On the \ref PCIe state events are received on the channel associated
 *       with the port rather than channel 0.
 ...
 * given port.
 *
 * \note This function is currently only compatible with \ref PCIe devices.
 ...
 *                          the measured link speed in 100 Kbits/s units, see
