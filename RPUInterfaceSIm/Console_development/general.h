 ...
 *
 * This is the type that will be used by createVirtualDevice to select a driver
 * to create a device instance on.
 *
 * @param driverId              Identifier for driver to be checked
 *
...
* Configuration APIs (#STAR_DEVICE_CONFIG_SUPPORTED), or whether it is a special
* device such as a Link Analyser or Conformance Tester
 ...
 * \ingroup virtual
 *
 * Sets the serial number of a given virtual device.
 *
 * @param     deviceId          The device to set the new serial number for
 * @param[in] newSerial         The serial number you wish to set in the virtual device.
 *                              This is a STAR_DEVICE_SERIAL_SIZE length char string including
 *                              the null terminator
 *
 * @return                  1 if the new serial number was set correctly, else 0
 */
int STAR_API_CC STAR_setVirtualDeviceSerialNumber(STAR_DEVICE_ID deviceId,
    _In_z_ char* newSerial);
 ...
 *
 * \bug This function does not currently work.
 *
