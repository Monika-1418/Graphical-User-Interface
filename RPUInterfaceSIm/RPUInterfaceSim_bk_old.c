...
int ColAmp[2][3]= {{0x4E, 0x54, 0x71},
                   {0x4F, 0x62, 0x71}}; 

int PGA[2][3] = {{0x04, 0xF5, 0xD6},
                 {0x08, 0xFD, 0xEA}};
...
void FormatData ()
{
  ...
  float dist,linemoment,pixmoment,sumi,centL,centP;
	  ...
	  if ((mode == 1)  && (flag_err != 1))  //Image mode
		       ...
		       for (k=0;k<framelength;k++)
		         {
		              ...
		              ff_write[j][k]=intensity + pRxBuffer[(2*(k+j*1024))+1]; 
		...
		wincentP[z] = centP;
		  ...
		  fprintf(txtfile,"\n");
 		  ...
 		  printf("\n\nWindow-%d...centl-%f , centp- %f, cpi-%d, sumi-%f, err-%d\n",testNum,wincentL[z],wincentP[z],wincpi[z],winsumi[z],flagcenterr);
		
                  fprintf(txtfile, "\n\nWindow-%d...centl-%f , centp- %f, cpi-%d, sumi-%f, err-%d, maxl-%d, maxp-%d\n\n\n",testNum,wincentL[z],wincentP[z],wincpi[z],winsumi[z],flagcenterr,xl,xp);
...
int FFdefaultValues()  //modified for spadex pds engineering model
{
	...
	if (l_mode==1)
	 {   
             ...
             // cmdval = 0x99;              	//8 bit mode, HK enabled, 6MHz Sobel Mode
	      pix_width=1;
         }
