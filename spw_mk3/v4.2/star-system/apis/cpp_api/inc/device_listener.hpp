/**
 * \file device_listener.hpp
 *
 * \brief Identifies a listener or call-back object which has been registered to
 *        receive notifications of device-related events, e.g. devices added or
 *        removed.
 *
 ...
 * receive notifications of device-related events, e.g. devices added or
 * removed, and which must subsequently be unregistered when device
...
#ifndef __STAR_DEVICE_LISTENER_CPP__
#define __STAR_DEVICE_LISTENER_CPP__
 ...
 * notifications are no longer required.
*/
class DeviceListener : STARSystemListener
{
private:
...
public:
    ...
    */
    DeviceListener() : listenerID(0)
    {
    }

    /**
    * Overloaded constructor, initialise with a given device listener ID.
    *
    * @param id The ID of the device listener.
    *
    * \note Listener objects are usually constructed within the C++ API and
    *       provided as return values to functions, rather than being
    ...
    *
    */
    explicit DeviceListener(STAR_DEVICE_LISTENER_ID id)
                        : listenerID(id)
    {
    ...
    *
    * The DeviceListener will be automatically unregistered if required,
    ...
    */
        ...
        if (pListenerReferenceCountandStatus)
        {
            ...
            if (refCount < 1)
            {
                ...
                if (pListenerReferenceCountandStatus->IsRegistered())
                {
                     ...
                     * it now */
                    STAR_unregisterDeviceListener(listenerID);
                }
    ...
    /**
    * Copy constructor - to allow copies of DeviceListener objects to be made
    * when being passed by value in functions or returned by value from
    * functions.
    *
    * When a copy of an existing DeviceListener is made, the copy
    * shares a reference to the existing DeviceListener. This is
    * rather than registering a new DeviceListener which is a
    * duplicate of that being copied. A reference counting mechanism
    * is employed to manage shared references to a device listener.
    */
    DeviceListener(const DeviceListener& listenerToCopyFrom) :
        STARSystemListener()
    {
    ...
    /**
    * Overloaded assignment operator - to allow instances of DeviceListener
    * objects to have their contents assigned to each other.
    *
    * When the contents of one DeviceListener object are assigned to another,
    * they will both share a reference to the same underlying device listener.
    * This is rather than a duplicate being made, which would involve
    * registering an entirely new DeviceListener. Any existing listener
    ...
    * is employed to manage shared references to a device listener.
    */
    DeviceListener& operator=(const DeviceListener& listenerBeingAssignedFrom)
    {
    ...
    *
    */
    STAR_DEVICE_LISTENER_ID GetListenerID()
    {
        return listenerID;
    }
    ...
    * registered to be called whenever a device is added or removed.
    *
    ...
    virtual int Unregister()
    {
        ...
        if (pListenerReferenceCountandStatus)
        {
            ...
            if (pListenerReferenceCountandStatus->IsRegistered())
            {
                ...
                if (STAR_unregisterDeviceListener(listenerID))
                {
