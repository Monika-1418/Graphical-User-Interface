...
class ImageAcquisitionApp(QMainWindow):
    ...
    def run_capture(self):
        ...
        script_path = os.path.join(script_dir, "gui", "run_capture.sh")
        ...
        try:
            ...
            # Ask for sudo password via GUI dialog
            password, ok = QInputDialog.getText(
                self, "Authentication Required",
                "Enter sudo password for RPU hardware access:",
                QLineEdit.Password
            )
            ...
            askpass_fd = tempfile.NamedTemporaryFile(
                mode='w', suffix='.sh', delete=False, prefix='askpass_'
            )
            askpass_fd.write(f'#!/bin/bash\necho "{password}"\n')
            askpass_fd.close()
            os.chmod(askpass_fd.name, stat.S_IRWXU)  # 700

            self._log("Sending command to RPU interface...")
            self._start_activity_animation()

            # Call the shell script with SUDO_ASKPASS set
            mode = "1" if "PDS" in self.mode_input.currentText() else "0"
            env = os.environ.copy()
            env['SUDO_ASKPASS'] = askpass_fd.name
            ...
            expected_image_path = os.path.join(save_path, "images", f"{file_name}_0.pgm")
